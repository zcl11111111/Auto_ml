...
data_set_re = pd.read_csv("Processed_data.csv")
...
for i in range(50):
    ...
    for train_data, test_data in kf.split(data_set_re):   
        ...
        predictor_re = TabularPredictor(label='Target feature').fit(train_data, num_stack_levels=2, num_bag_folds=3)  # Fit models for 120s
        ...
        MAE = mae(test_data.loc[:,'Target feature'],result_test)
        MSE = mse(test_data.loc[:,'Target feature'],result_test)
        R2 = r2(test_data.loc[:,'Target feature'],result_test)
        MAPE = mape(test_data.loc[:,'Target feature'],result_test)
...
ax.plot(result_train, train_data["Target feature"],"D",color='aquamarine', label='Train');
ax.plot(result_test, test_data["Target feature"],"*", color='navy', label='Test');
plt.plot([3,9], [3,9], '--',color='grey');
plt.xlim((3,9))
plt.ylim((3,9))
